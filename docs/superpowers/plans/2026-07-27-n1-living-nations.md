...
| 4 | Per-capita food production and consumption, trade-route income, city growth and capacity rates, famine population loss, stability drift bounds, culture gain, **initial stability and initial culture**, and prosperity weights (population 0.30, production 0.25, wealth 0.20, stability 0.15, culture 0.10) |
...
- `prosperity.ts` exports `computeProsperity(nation, livingField): ProsperityScore`, clamping negative raw inputs to zero and normalising each component as a `log1p` ratio against the shared maximum for that component across the living field. The target nation participates in the maximum even when omitted from `livingField`; a non-positive maximum produces zero. Components stay within 0..1 and the weighted total stays within 0..1000.
...
- **The five components are the normalised 0..1 log ratios against their maxima in the same living-field snapshot, not raw metrics.** The client is forbidden from reconstructing the normalisation itself — so pin it here and assert it in the tests. `total` is the weighted sum times 1000. The client renders each component's contribution as `weight × component × 1000` and displays `total` verbatim; it never sums contributions into a total of its own.
- Tests: pipeline order observable through the ledger; ledger sums match state deltas; reordering the input array does not change any nation's result; removing a rival can change another nation's score because it changes the living field; zero maxima, negative inputs, and an omitted target remain inside the public ranges; a full famine cycle behaves as specified.
