...
**The banner palette's ΔE 26 acceptance bar is unreachable from any world the generator can make.** Closed by C1-1b (`e7b17ab`), and the closing measurement retracted the reason first recorded here. This paragraph originally said the bar "only binds at six nations or more"; that was an inference, and it is false. The bar can only change an outcome when a fallback slot contends with its primary neighbour, which takes at least five *chromatic* polities — and only four of the eight archival templates clear the chroma floor of 18 (gold 47.1, ember 37.9, moss 30.5, river 19.5; sable 7.8, salt 8.6, thorn 13.9, ivory 15.7 fall below it), so at most four ring hues are ever claimed. C1-1b proved it by dumping the slot assignment for all 163 colour sets the generator can draw at counts four through eight under `MIN_SEPARATION` 26 and 0 and diffing: not one assignment differs, and the worst-case floors are identical to four decimal places. That is stronger than comparing floors alone, because it also rules out the bar shifting an assignment without moving the floor.

The bar is therefore guarded by contract rather than by a generated world. `refuses a hue-exact slot whose neighbour is already taken` feeds the two ring colours of a below-bar ring pair — olive/moss at ΔE 24.62 and violet/plum at ΔE 20.75, each a primary against a fallback — so both nations want that exact pair and only the bar can refuse the second. Verified load-bearing independently: with `MIN_SEPARATION` at 0, that test alone fails, and the 5–8 nation floor enumeration alongside it stays green. Use synthetic colours for the same reason if the ring is ever re-tuned; a ninth template or a populated override table is what would make the bar reachable for real.
