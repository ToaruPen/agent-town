...
| Retire `heldOrderNote` and `commitSlot.detail` | `client/src/ui/seasonReportViewModel.ts`, `nationDashboardViewModel.ts`, both panels, `.season-report__held` CSS | Since `0876200` both are permanently `null` on every path. **Owner decided retirement 2026-08-18** — a future held-state display gets designed fresh rather than inheriting this one. **Dispatched** on `chore-retire-held-ui`, with the one sanctioned exception to "never delete a test": assertions that exist solely to pin the retired fields go with them; mixed assertions keep their surviving half |
