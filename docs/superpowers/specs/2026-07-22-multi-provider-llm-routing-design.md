...
- `トネリコ`はClaude、その他の現在および将来の住民はCodex、という構成を1つの環境変数で指定できる。
...
# トネリコだけClaude、その他と将来の移住者はCodex
LLM_ROUTES='トネリコ:claude,*:codex'

# 選択した2人を異なるプロバイダーへ送る
LLM_AGENTS='トネリコ,シラカバ'
LLM_ROUTES='トネリコ:claude,シラカバ:codex'
```
...
- `LLM_ROUTES='トネリコ:claude,*:codex'`で、トネリコの要求はClaudeだけへ、それ以外はCodexだけへ送られる。
- 将来の移住者が`*`ルートでCodexへ割り当てられる。
- Claudeが失敗してもCodexへ切り替わらず、トネリコだけがFakePlannerへ退避する。
